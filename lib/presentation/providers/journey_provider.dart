...
class JourneyProvider with ChangeNotifier, LoadingStateProviderMixin, MultiLoadingProviderMixin {
  ...
  Future<void> autoGenerateTestIfNeeded(int journeyId) async {
    ...
    try {
      ...
      setError('Tạo bài test thất bại: ${e.toString()}');
    } finally {
  ...
  Future<AssessmentTestDto?> getAssessmentTest({
    ...
    return await performAsyncFor<AssessmentTestDto>(
      'getTest',
      () async {
  ...
  Future<TestResultDto?> submitTest({
    ...
    return await performAsyncFor<TestResultDto>(
      'submitTest',
      () async {
